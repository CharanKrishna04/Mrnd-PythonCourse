...
def custom_sort(input):
    #input= sorted(sorted(input,key=operator.itemgetter(2),reverse=False),key=operator.itemgetter(1),reverse=True)
    if(input==None):
        input=[]
    input.sort(key=operator.itemgetter(2), reverse=False)
    input.sort(key=operator.itemgetter(1), reverse=True)
...
def max_vowels(words):
    if(words==None or words==[]):
        return None
    return max(words, key=my_count_vowels)
